...
from models import Transformer ,iTransformer,Crossformer,FEDformer
from models import Informer,TimesNet,PatchTST
from models import Autoformer
from models import Transformer ,MKAN_iTransformer
...
def cal_eval(y_real, y_pred):
    ...
    return df_eval
df = pd.read_csv('./data/newdata_sudan.csv')
...
config = Config()
print("zhibozhenpanghhhhhhhhhhhhhhhh")
model_type = 'MKAN_iTransformer'
...
df.to_csv('loss_mkanitransformer.csv', index=False, encoding='utf-8')
print("loss.csv文件中")
...
result_df.to_csv('MKAN_iTransformer真实值与预测值.csv', index=False, encoding='utf-8')
# 打印保存成功的消息
